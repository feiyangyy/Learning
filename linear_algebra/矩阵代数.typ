...
== 双射
如果映射#f 既是满射也是单射，则$forall y in #bss$ 都能在#bs 找到#highlight()[唯一一个]原象。此时称#f 是双射
...
1. 假设$(a+b)^n = sum_(k=0)^n C_n^k a^k b^(n-k)$成立，易验证n = 1时, 等式成立，现在假设其在n=N 时成立
2. 我们证明(n= N+1)时也成立，证明如下:
$
(a+b)^(N+1) = (a+b)[sum_k C_N^k a^k b^(n-k)] = a sum_k C_N^k a^k b^(n-k) + b sum_k C_N^k a^k b^(n-k) = 
$ --- 先跳过 时间不够了 // TODO
